...
export function setupSidebar({ sidebarContainerId = 'sidebar-container' } = {}) {
  ...
  function openSidebarOverlay(desktopOverlay = false) {
    // mark container open; css reacts to .open and .sidebar-overlay-active
    container.classList.add('open');
    // ensure overlay-active is present for overlay mode (mobile and explicit desktop overlay)
    ...
    // position overlay panel below topbar with a small gap
    const gap = 8;
    sidebar.style.top = `${topbarH + gap}px`;
    sidebar.style.left = `18px`;

    // make page unscrollable while overlay open
    document.documentElement.classList.add('no-scroll');
    document.body.style.overflow = 'hidden';

    // apply sizing for desktop overlay if view large
    if (!isMobile()) applyDesktopOverlaySizing();

    // ensure overlay visuals appear
  ...
  function closeSidebarOverlay() {
    ...
    // reset inline styles applied for overlay positioning
    overlay.style.top = '';
  ...
  function toggleSidebarMobile() {
    // mobile always uses overlay semantics
  ...
  function toggleCollapsedDesktop() {
    // Toggle collapsed (dock) state on both sidebar and container so CSS sibling selectors can react
    const collapsedNow = !sidebar.classList.contains('collapsed');
    applyCollapsed(collapsedNow);

    // Ensure overlay mode is disabled when docking (non-overlay docked behavior)
    ...
    // Ensure container.open is not left set for docked toggles
    container.classList.remove('open');
  ...
  try {
    ...
    window.addEventListener('resize', () => {
      if (isMobile()) {
        // ensure overlay mode and not collapsed dock
